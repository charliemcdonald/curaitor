...
export function Artwork(card: Art) {
  return (
    <div onClick={() => updateResult(card)} className="m-0 mt-4 mb-4 relative w-[330px]">
      <img src={frame2.src} className="relative h-[330px] w-[400px]"/>
      <img src={card.image.src} className='h-[250px] w-[200px] absolute top-[40px] right-[65px] object-cover'/>
    </div>
  );
}
    ...
    <div id="curaitor" className="flex items-center h-3/4">
      <div id="gallery" className="z-40">
        <form className="items-center"> {/* Sumbission Form to Select AI art*/}
          <div id="artwork" className="flex flex-row justify-between">
          ...
          <div id="submit" className='flex flex-col items-center justify-center m-10'>
            <button type="button" className="p-1 border-2 block" onClick={selectArtwork}>Submit</button>
          </div>
        </form>
      </div>
      <div id="resultAI" className="transition-opacity duration-1000 opacity-0 absolute items-center">
        <div className="w-[550px] h-[350px] drop-shadow-lg bg-slate-100 rounded-[2em] flex flex-col items-center justify-center">
          ...
          <button className="font-bold text-xl border-2 rounded-xl p-4" onClick={() => window.location.reload()}>Play Again?</button> 
        </div>
      </div>
